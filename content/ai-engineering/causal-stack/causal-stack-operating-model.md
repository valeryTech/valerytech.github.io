...
The old fault family index recognized compositional AI systems, including retrieval, memory, tools, state, validators, policy layers, and orchestration. But it still had a problem: it grouped failures by **conceptual fault type**, while the new `product-fault-families.md` groups them by **product debugging surface**.

That mixture is not fatal, because the old file explicitly says families are broad views. But it makes the taxonomy less useful for product debugging. A product engineer seeing an incident needs to know:

```
Where do I inspect first?
Which artifact failed?
Which trace should exist?
Which eval reveals it?
Which control should catch it?
```
...
For the detailed Layer 0 taxonomy, see [layer-0-natural-language-properties.md]({{< ref "ai-engineering/causal-stack/layer-0-natural-language-properties" >}}).
